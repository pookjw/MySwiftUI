        ...
        fileprivate func updateItemViewAsync(
            ...
            for index in list_1.items.indices {
                ...
                if requirements_1.contains(.unknown1) {
                    ...
                    if copy_5.version == copy_10.version {
                        // <+3932>
                        if case .effect(_, let list_3) = copy_5.value {
                            // <+3960>
                            for item in list_3.items {
                                self.viewCache.index.skip(item: item)
                            }
                        ...
                        // <+3496>
                        let flag_1: Bool // true -> <+4220> / false -> <+3588>
                        if requirements_1.contains(.unknown2) {
                            // <+4220>
                            flag_1 = true
                        } else {
                            ...
                            if copy_14.features.contains(.required) {
                                // <+4220>
                                flag_1 = true
                            } else {
                                // <+3588>
                        ...
                        if flag_1 {
                            ...
                            if requirements_1.contains(.unknown0) {
                                // <+4628>
                                guard let _d8 = self.updateItemViewAsync(platform: platform, oldItem: copy_5, oldState: &copy_8, newItem: copy_10, newState: &copy_12) else {
                                    // <+9812>
                                    self.viewCache.index.leave(index: oldIndex)
                                    return nil
                                ...
                                // <+4224>
                                // sp + 0x720
                                let copy_14 = copy_5.value
                                // sp + 0x740
                                let copy_15 = copy_10.value
                                
                                // <+4336>
                                if
                                    case .effect(let effect_3, let list_3) = copy_14,
                                    case .effect(let effect_4, let list_4) = copy_15
                                {
                                    // <+4360>
                                    // sp + 0x860
                                    let copy_16 = list_3
                                    ...
                                    guard let _d8 = self.updateAsync(
                                        platform: platform,
                                        oldList: copy_16,
                                        oldParentState: &copy_8,
                                        newList: copy_17,
                            ...
                            let copy_14 = copy_5
                            // sp + 0x810
                            let features_1 = copy_14.features
                            // sp + 0x860
                            let copy_15 = copy_10
                            // sp + 0x5c0
                            let features_2 = copy_15.features
                            
                            // <+3696>
                            guard features_1 == features_2 else {
                                // <+9800>
                                self.viewCache.index.leave(index: oldIndex)
                                return nil
                            }
                            ...
                            // <+3716>
                            if case .effect(let effect_3, let list_3) = copy_5.value {
                                // <+3736>
                                self.viewCache.index.skip(list: list_3)
                                self.viewCache.index.skip(effect: effect_3)
                                // <+3916>
                    ...
                    if requirements_1.contains(.unknown0) {
                        // <+1544>
                        guard let _d8 = unsafe self.updateItemViewAsync(platform: platform, oldItem: copy_5, oldState: &copy_8, newItem: copy_10, newState: &copy_12) else {
                            // <+9644>
                            self.viewCache.index.leave(index: oldIndex)
                            return nil
                        }
                        ...
                        // <+5560>
                        // sp + 0x500 (x19, x21, x23, x28)
                        let copy_14 = copy_5.value
                        // sp + 0x520 (x27, x20, x24, x26)
                        let copy_15 = copy_10.value
                            ...
                            case .effect(let effect_4, let list_4) = copy_15
                        {
                            ...
                            if list_3.items.count == list_4.items.count {
                                ...
                                for index in list_3.items.indices {
                                    ...
                                    // <+6000>
                                    guard (item_1.identity == item_3.identity) && item_4.matchesTopLevelStructure(of: item_2) else {
                                        // <+7224>
                                        self.viewCache.index.leave(index: oldIndex)
                                        return nil
                                    }
                                    ...
                                    // <+6052>
                                    // sp + 0x40
                                    let oldIndex_1 = self.viewCache.index.enter(identity: item_4.identity)
                                    
                                    // <+6148>
                                    let d10 = unsafe self.viewCache.prepare(item: &item_2, platform: platform, parentState: &copy_8)
                                    self.viewCache.index = oldIndex_1
                                    let d11 = unsafe self.viewCache.prepare(item: &item_4, platform: platform, parentState: &copy_12)
                    ...
                    // inlined
                    // w19
                    let features_1 = copy_10.features
                    ...
                    if features_1.contains(.required) {
                        ...
                        if requirements_1.contains(.unknown0) {
                            ...
                            {
                                // <+5696>
                                guard list_3.items.count == list_4.items.count else {
                                    // <+6704>
                                    self.viewCache.index.leave(index: oldIndex)
                                    return nil
                                }
                        ...
                        // inlined
                        // w27
                        let features_2 = copy_5.features
                        // inlined
                        // <+7596>
                        // w19
                        let features_3 = copy_10.features
            ...
            for index in list_3.items.indices {
                ...
                // x20 -> sp + 0xa60
                // sp + 0xe40
                let copy_8 = unsafe oldState
                // sp + 0xc00
                var copy_9 = unsafe oldState
                // sp + 0x910
                let copy_10 = self.viewCache.index
                ...
                // x28 -> sp + 0x810
                // sp + 0xf90
                let copy_12 = unsafe newState
                // sp + 0xab0
                var copy_13 = unsafe newState
                // sp + 0x720
                let copy_14 = self.viewCache.index
                ...
                if requirements_1.contains(.unknown1) {
                    ...
                    if copy_6.version == copy_11.version {
                        // <+12184>
                        if case .effect(_, let list_3) = copy_6.value {
                            // <+12220>
                            for item in list_3.items {
                                self.viewCache.index.skip(item: item)
                            }
                        ...
                        if requirements_1.contains(.unknown2) {
                            ...
                            if requirements_1.contains(.unknown0) {
                                ...
                                // <+12492>
                                // sp + 0x460 (x21, x22, x26, x20)
                                let copy_15 = copy_6.value
                                // sp + 0x480 (x23, x24, x27, x28)
                                let copy_16 = copy_11.value
                                
                                if
                                    case .effect(let effect_3, let list_3) = copy_15,
                                    case .effect(let effect_4, let list_4) = copy_16
                                {
                                    // <+12628>
                                    guard let _d8 = unsafe self.updateAsync(
                                        platform: platform,
                                        oldList: list_3,
                                        oldParentState: &copy_9,
                                        newList: list_4,
                                        newParentState: &copy_13
                                    ) else {
                                        // <+16520>
                                        self.viewCache.index.leave(index: oldIndex)
                                        return nil
                            ...
                            // <+11796>
                            // sp + 0x810
                            let features_1 = copy_11.features
                            ...
                            if features_1.contains(.required) {
                                ...
                                // <+11864>
                                // sp + 0x500
                                let features_2 = copy_6.features
                                // sp + 0x580
                                let features_3 = copy_11.features
                                ...
                                // <+11988>
                                if case .effect(let effect_3, let list_3) = copy_6.value {
                                    // <+12008>
                                    for item in list_3.items {
                                        self.viewCache.index.skip(item: item)
                                    }
                    ...
                    if requirements_1.contains(.unknown2) {
                        ...
                        if requirements_1.contains(.unknown0) {
                            ...
                            // <+14044>
                            // sp + 0x910 (x23, x19, x26, x20)
                            let copy_15 = copy_6.value
                            // sp + 0x930 (x24, x21, x27, x28)
                            let copy_16 = copy_11.value
